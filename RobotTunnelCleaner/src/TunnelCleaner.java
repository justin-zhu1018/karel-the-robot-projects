...
public class TunnelCleaner implements Directions {
	...
	private void cleanRoom() {
		...
		while(fizz.facingNorth()==true){
			...
			if(fizz.frontIsClear()==true){
				...
				ave++;
				aven=ave-1;
				System.out.println("Aven:"+aven+" street: "+st);
				street=st;
				Start();
			}
			if(fizz.frontIsClear()==false){
	...
	private void reset() {
		...
		if(fizz.areYouHere(st, aven)==true){
			if(fizz.areYouHere(st, aven)){
				fizz.turnLeft();
				fizz.turnLeft();
				fizz.move();
				fizz.turnLeft();
			...
			while(fizz.anyBeepersInBeeperBag()==true){
				fizz.putBeeper();
			}
		
			if(fizz.anyBeepersInBeeperBag()==false){
				while(fizz.areYouHere(st, aven)==false){
					while(fizz.frontIsClear()==true){
						fizz.move();
						reportStats();
					}
